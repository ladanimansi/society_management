...
class BlockAdminView extends GetView<BlockAdminController> {
  const BlockAdminView({super.key});
  ...
  Widget build(BuildContext context) {
    return Obx(() {
      if (controller.isLoading.value) {
        return Scaffold(
          appBar: AppBar(
            title: Text('BLOCK_ADMIN-TITLE'.tr),
      ...
      WidgetsBinding.instance.addPostFrameCallback((_) {
        // Automatically open the form if arguments request it
        if (Get.arguments != null && Get.arguments is Map && Get.arguments['action'] == 'create') {
          final blockName = Get.arguments['blockName'];
          Get.arguments.clear(); // Ensure it only opens once
          
          if (controller.blocks.isNotEmpty) {
            controller.prepareCreate();
            if (blockName != null && controller.blocks.contains(blockName)) {
              controller.selectedBlockName.value = blockName;
            }
            BlockAdminView.openAdminForm(context, controller);
          }
        }
      });
        ...
        floatingActionButton: FloatingActionButton.extended(
          ...
          onPressed: () {
            if (controller.blocks.isEmpty) {
              Get.snackbar('Error', 'No blocks available to assign'.tr);
              return;
            }
            controller.prepareCreate();
            BlockAdminView.openAdminForm(context, controller);
          ...
          label: Text('BLOCK_ADMIN-ADD_ADMIN'.tr, style: TextStyle(color: Colors.white)),
        ),
        body: () {
          if (controller.admins.isEmpty) {
            return Center(child: Text('BLOCK_ADMIN-NO_ADMIN'.tr));
          }

          final Map<String, List<BlockAdminModel>> groupedAdmins = {};
          for (final admin in controller.admins) {
            final key = admin.blockName.isEmpty
                ? 'BLOCK_ADMIN-UNASSIGNED'.tr
                : admin.blockName;
            groupedAdmins.putIfAbsent(key, () => []);
            groupedAdmins[key]!.add(admin);
          }
          final blockGroups = groupedAdmins.entries.toList()
          ...
          return ListView.builder(
            ...
            itemBuilder: (context, index) {
              final group = blockGroups[index];
              final blockName = group.key;
  ...
  static void openAdminForm(BuildContext context, BlockAdminController controller) {
                ...
                children: [
                  Text(
                    controller.editingAdminId.value == null
                        ? 'BLOCK_ADMIN-CREATE_ADMIN'.tr
                        : 'BLOCK_ADMIN-EDIT_ADMIN'.tr,
                  ...
                  DropdownButtonFormField<String>(
                    value: controller.selectedBlockName.value,
                    decoration: InputDecoration(
                      labelText: 'BLOCK_ADMIN-SELECT_BLOCK'.tr,
                    ...
                    items: controller.blocks
                        .map(
                          (block) => DropdownMenuItem<String>(
                            value: block,
                            child: Text("${'BUILDING-BLOCK'.tr} $block"),
                          ),
                        )
                        .toList(),
                    onChanged: (value) =>
                        controller.selectedBlockName.value = value,
                  ),
                  const SizedBox(height: 10),
                  Row(
                    children: [
                  ...
                  SizedBox(
                    ...
                    child: ElevatedButton.icon(
                      ...
                      onPressed: controller.isSaving.value
                          ? null
                          : () async {
                              final done = await controller.saveBlockAdmin();
                              if (done) {
                                Get.back();
                              }
                            },
