...
class SocietyAdminBlock {
  ...
  List<Map<dynamic, dynamic>> activeAdmins;

  SocietyAdminBlock({required this.name, required this.floors, this.activeAdmins = const []});
}
...
class SocietyAdminHomeController extends GetxController {
  ...
  void _listenBlockAdmins() async {
    ...
    _adminSubscription = FirebaseDatabase.instance.ref('block_admins').onValue.listen((event) {
      ...
      final adminList = <Map<dynamic, dynamic>>[];
      ...
      for (var block in blocks) {
        block.activeAdmins = adminList.where((a) => a['blockName'] == block.name).toList();
      }
      blocks.refresh();
    });
  }
